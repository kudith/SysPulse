...
class CommandQueue {
  ...
  processQueue() {
    ...
    this.running++;
    const { connection, command, callback, socket, sessionId, socketId } = this.queue.shift();

    console.log(`Command Queue: Processing command: ${command} (running: ${this.running}, queued: ${this.queue.length})`);
    ...
    try {
      ...
      sshClient.exec(command, (err, stream) => {
        ...
        stream.on('error', (err) => {
          console.error(`${socketId}: Stream error during command: ${err.message}`);
          if (typeof callback === 'function') {
            callback({ error: `Stream error: ${err.message}`, output: output, errorOutput });
          }
      ...
      console.error(`${socketId}: Exception executing command: ${error.message}`);
      if (typeof callback === 'function') {
...
io.on('connection', (socket) => {
  ...
  if (sessionId) {
    console.log(`Client ${socket.id} attempting to reconnect with session ID: ${sessionId}`);
    ...
    if (sshConnections.has(sessionId)) {
      ...
      console.log(`Client ${socket.id} reconnected to existing session ${sessionId}`);
      ...
      if (existingConnection.sshStream && !existingConnection.sshStream.destroyed) {
        console.log(`${socket.id}: Reinitializing SSH stream for session ${sessionId}`);
        ...
        existingConnection.sshStream.on('data', (data) => {
          ...
          if (data && data.length > 0) {
            ...
            console.log(`${socket.id}: Forwarding ${data.length} bytes of data to client`);
            
            // Wrap in try-catch to avoid any errors breaking the connection
            try {
              socket.emit('ssh-data', stringData);
            } catch (err) {
              console.error(`${socket.id}: Error sending data to client: ${err.message}`);
            }
          }
        });
        existingConnection.sshStream.stderr.on('data', (data) => {
          ...
          if (data && data.length > 0) {
            try {
              ...
              console.error(`${socket.id}: Error sending error data to client: ${err.message}`);
            }
          }
        });
      }
  ...
  socket.on('ssh-connect', (data) => {
    console.log(`SSH connection requested from ${socket.id} to ${data.username}@${data.host}:${data.port}`);
    ...
    if (!processedPrivateKey.includes('-----BEGIN') && !processedPrivateKey.includes('-----END')) {
      console.error(`${socket.id}: Invalid private key format - missing BEGIN/END markers`);
      socket.emit('ssh-error', { message: 'Invalid private key format - please use PEM format with BEGIN/END markers' });
      return;
    }
    ...
    const hardAuthTimeout = setTimeout(() => {
      if (!connection.authenticated) {
        console.log(`${socket.id}: Hard authentication timeout reached, forcibly ending connection`);
        socket.emit('ssh-error', { message: 'Authentication timeout - connection terminated' });
        ...
        if (sshClient) {
          try {
            ...
            console.error(`${socket.id}: Error during forced disconnection: ${err.message}`);
          }
        }
    ...
    sshClient.on('ready', () => {
      ...
      console.log(`${socket.id}: SSH connection established to ${username}@${host}:${port}`);
    ...
    sshClient.on('close', (hadError) => {
      console.log(`${socket.id}: SSH client closed${hadError ? ' with error' : ''}`);
      socket.emit('ssh-closed', { message: `SSH connection closed${hadError ? ' with error' : ''}` });
    ...
    try {
      ...
      connectConfig.authHandler = function(methodsLeft, partialSuccess, callback) {
        ...
        if (!methodsLeft || !Array.isArray(methodsLeft)) {
          console.log(`${socket.id}: WARNING - Auth methods list is ${methodsLeft === null ? 'null' : 'invalid'}, defaulting to publickey`);
        ...
        console.log(`${socket.id}: Auth methods available: ${methodsLeft.join(', ')}`);
        socket.emit('ssh-data', `\r\n\x1b[33mAuth methods available: ${methodsLeft.join(', ')}\x1b[0m\r\n`);
        
        // Prioritas pemilihan metode autentikasi
        if (methodsLeft.includes('publickey')) {
          console.log(`${socket.id}: Selecting 'publickey' authentication method`);
          return callback('publickey');
        }
        
        if (methodsLeft.includes('keyboard-interactive')) {
          console.log(`${socket.id}: Selecting 'keyboard-interactive' authentication method`);
          return callback('keyboard-interactive');
        }
        
        if (methodsLeft.includes('password')) {
          console.log(`${socket.id}: Selecting 'password' authentication method`);
          return callback('password');
        }
        
        // Jika tidak ada metode yang cocok, gunakan yang pertama tersedia
        console.log(`${socket.id}: No preferred auth methods available, using: ${methodsLeft[0] || 'none'}`);
      ...
      sshClient.on('keyboard-interactive', (name, instructions, lang, prompts, finish) => {
        console.log(`${socket.id}: Keyboard-interactive auth initiated: ${prompts.length} prompts`);
        ...
        if (passphrase && passphrase.length > 0 && prompts.length > 0) {
          console.log(`${socket.id}: Attempting keyboard-interactive with passphrase`);
      ...
      connectConfig.debug = (message) => {
        ...
        const isNoisy = debugMsg.includes('keepalive@openssh.com') || 
        ...
        if (debugMsg.includes('Authentication failed') || 
      ...
      let watchdogTimer = setInterval(() => {
        if (!connection.authenticated) {
          console.log(`${socket.id}: Watchdog check - connection still authenticating...`);
        }
      ...
      console.log(`${socket.id}: Attempting SSH connection to ${username}@${host}:${port}`);
  ...
  socket.on('ssh-execute-command', async (data, callback) => {
    ...
    if (!sessionId || !sshConnections.has(sessionId)) {
      console.warn(`${socket.id}: Attempted to execute command without an active SSH connection`);
      if (typeof callback === 'function') {
    ...
    if (!sshClient || !connection.authenticated) {
      console.warn(`${socket.id}: SSH client not authenticated for command execution`);
      if (typeof callback === 'function') {
  ...
  socket.on('ssh-restart-shell', () => {
    ...
    if (!sessionId || !sshConnections.has(sessionId)) {
      console.warn(`${socket.id}: Attempted to restart shell without an active SSH connection`);
    ...
    if (!sshClient || !sshClient.config) {
      console.error(`${socket.id}: SSH client not properly initialized for restart`);
      socket.emit('ssh-error', { message: 'SSH client not properly initialized' });
      return;
    }
  ...
  function createShell(clientSocket, connection) {
    if (!connection || !connection.sshClient) {
      console.error(`${clientSocket.id}: Cannot create shell - SSH client not available`);
    ...
    new Promise((resolve, reject) => {
      ...
      console.log(`${clientSocket.id}: New shell stream created successfully`);
      connection.sshStream = stream;
      
      // Update last activity
      connection.lastActivity = new Date();
      
      // Set up stream handlers with proper buffering
      let dataBuffer = '';
      let dataTimeout;
      
      // Efficient data handling with batching to avoid UI rendering issues
      stream.on('data', (data) => {
        connection.lastActivity = new Date();
        ...
        // Send data immediately for small chunks, batch for larger chunks
        if (data.length < 1024) {
          console.log(`${clientSocket.id}: Sending ${data.length} bytes of data to client (immediate)`);
          try {
            clientSocket.emit('ssh-data', dataBuffer);
            dataBuffer = '';
          } catch (err) {
            console.error(`${clientSocket.id}: Error sending data to client: ${err.message}`);
          }
        } else {
          // For large data, batch and send after a brief delay to avoid overwhelming the client
          dataTimeout = setTimeout(() => {
            if (dataBuffer.length > 0) {
              console.log(`${clientSocket.id}: Sending ${dataBuffer.length} bytes of batched data to client`);
              try {
                clientSocket.emit('ssh-data', dataBuffer);
                dataBuffer = '';
              } catch (err) {
                console.error(`${clientSocket.id}: Error sending batched data to client: ${err.message}`);
              }
            }
      ...
      stream.on('close', (code, signal) => {
        ...
        if (dataBuffer.length > 0) {
          try {
            ...
            console.error(`${clientSocket.id}: Error sending final data to client: ${err.message}`);
          }
        }
        
        console.log(`${clientSocket.id}: SSH stream closed. Code: ${code}, Signal: ${signal}`);
        clientSocket.emit('ssh-closed', { 
          message: `SSH connection closed (${code ? 'code: '+code : ''} ${signal ? 'signal: '+signal : ''})`.trim() 
        });
      });
      
      // Execute initial commands to check system resources
      // Note: this runs AFTER successful connection to avoid race conditions
      clientSocket.emit('ssh-data', '\r\n\x1b[32m# Connection successful, initializing system monitoring...\x1b[0m\r\n');
      ...
      console.error(`${clientSocket.id}: Failed to create shell: ${err.message}`);
      clientSocket.emit('ssh-error', { message: `Failed to create shell: ${err.message}` });
    });
  }
  ...
  function setupThrottledMonitoring(clientSocket, connection) {
    ...
    const checkResources = () => {
      ...
      connection.sshStream.write('echo "\\n--- System Status (`date`) ---\\n"; uptime; echo "\\n";\n');
    };
  ...
  socket.on('ssh-resize', (data) => {
    ...
    if (!sessionId || !sshConnections.has(sessionId)) {
      console.warn(`${socket.id}: Attempted to resize without an active SSH connection`);
      return;
    }
    ...
    try {
      ...
      if (!cols || !rows || cols <= 0 || rows <= 0) {
        console.warn(`${socket.id}: Invalid terminal dimensions: ${cols}x${rows}`);
        return;
      }
  ...
  socket.on('ssh-input', (data) => {
    ...
    if (!sessionId || !sshConnections.has(sessionId)) {
      console.warn(`${socket.id}: Attempted to send data without an active SSH connection`);
      socket.emit('ssh-error', { message: 'No active SSH connection' });
      return;
    }
    ...
    if (sshStream && !sshStream.destroyed) {
      ...
      try {
        ...
        socket.emit('ssh-error', { message: `Error sending data: ${err.message}` });
      }
    } else if (connection.sshClient && connection.sshClient.config && connection.authenticated) {
      console.log(`${socket.id}: Stream not available, attempting to recreate shell`);
      createShell(socket, connection);
      ...
      setTimeout(() => {
        if (connection.sshStream && !connection.sshStream.destroyed) {
          try {
            ...
            console.error(`${socket.id}: Error writing to recreated stream: ${err.message}`);
          }
        }
      }, 500);
    } else {
  ...
  socket.on('ssh-refresh-connection', () => {
    ...
    if (!sessionId || !sshConnections.has(sessionId)) {
      console.warn(`${socket.id}: Attempted to refresh connection without an active SSH connection`);
      socket.emit('ssh-error', { message: 'No active SSH connection' });
      return;
    }
    ...
    if (connection.sshStream && !connection.sshStream.destroyed) {
      ...
      console.log(`${socket.id}: No stream available, creating new shell on refresh`);
      createShell(socket, connection);
    }
  });
  ...
  function cleanupConnection(socketId, sessionId) {
    ...
    if (connection.sshStream && !connection.sshStream.destroyed) {
      console.log(`${socketId}: Destroying SSH stream for session ${sessionId}`);
      try {
...
setInterval(() => {
  ...
  console.log(`Memory usage: RSS: ${Math.round(memoryUsage.rss / 1024 / 1024)}MB, Heap Used: ${Math.round(memoryUsage.heapUsed / 1024 / 1024)}MB / ${Math.round(memoryUsage.heapTotal / 1024 / 1024)}MB`);
  ...
  if (memoryUsage.heapUsed > 800 * 1024 * 1024) { // 800MB
    ...
    for (const [sessionId, connection] of sshConnections.entries()) {
      if (!connection.authenticated) {
        console.log(`Memory cleanup: removing unauthenticated session ${sessionId}`);
...
setInterval(() => {
  ...
  console.log(`Session cleaner: checking ${sshConnections.size} active sessions`);
  ...
  for (const [sessionId, connection] of sshConnections.entries()) {
    ...
    if (elapsedMs > SESSION_EXPIRY_MS) {
      console.log(`Session cleaner: removing inactive session ${sessionId} (${connection.username}@${connection.host}), inactive for ${Math.floor(elapsedMs/1000/60)} minutes`);
...
// Start the server
const PORT = process.env.SSH_SERVER_PORT || 3001;
const HOST = process.env.SSH_SERVER_HOST || 'localhost';
